...
const TABS = ["All", "Full Stack", "AI", "Frontend", "Mobile App"];
...
const RecentProjects = () => {
  ...
  return (
    <section className="relative w-full bg-[#020602] py-24 px-4 md:px-8 font-sans selection:bg-emerald-500/30 overflow-hidden">
      
      {/* --- STATIC BACKGROUND (No Global Hover) --- */}
          ...
          <div className="flex flex-wrap gap-2">
            {TABS.map((tab) => (
              <button
                ...
                className={cn(
                  "relative px-4 py-2 rounded-full text-xs md:text-sm font-medium transition-all duration-300 z-10 overflow-hidden border",
                  activeTab === tab 
...
// --- ISOLATED CARD COMPONENT (With Local Spotlight) ---
const ProjectCard = ({ project }) => {
  ...
  return (
    ...
    >
      ...
      {/* 1. LOCAL SPOTLIGHT (Only visible on THIS card when hovered) */}
      <motion.div
      ...
      <div className="absolute inset-0 p-6 flex flex-col justify-end z-20">
        ...
        <div className="absolute top-6 left-6 right-6 flex justify-between items-start">
           ...
           <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-emerald-400">
             <Sparkles className="w-4 h-4 animate-pulse" />
           </div>
        </div>

        {/* Bottom: Details (Slide Up Effect) */}
